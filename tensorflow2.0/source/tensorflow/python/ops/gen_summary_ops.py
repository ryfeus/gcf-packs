...
@_kwarg_only
def CloseSummaryWriter(writer):
  return close_summary_writer(writer=writer)
tf_export("raw_ops.CloseSummaryWriter")(CloseSummaryWriter)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CreateSummaryDbWriter(writer, db_uri, experiment_name, run_name, user_name):
  return create_summary_db_writer(writer=writer, db_uri=db_uri, experiment_name=experiment_name, run_name=run_name, user_name=user_name)
tf_export("raw_ops.CreateSummaryDbWriter")(CreateSummaryDbWriter)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CreateSummaryFileWriter(writer, logdir, max_queue, flush_millis, filename_suffix):
  return create_summary_file_writer(writer=writer, logdir=logdir, max_queue=max_queue, flush_millis=flush_millis, filename_suffix=filename_suffix)
tf_export("raw_ops.CreateSummaryFileWriter")(CreateSummaryFileWriter)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def FlushSummaryWriter(writer):
  return flush_summary_writer(writer=writer)
tf_export("raw_ops.FlushSummaryWriter")(FlushSummaryWriter)
...
def import_event(writer, event, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "ImportEvent", name, _ctx._post_execution_callbacks, writer, event)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ImportEvent(writer, event):
  return import_event(writer=writer, event=event)
tf_export("raw_ops.ImportEvent")(ImportEvent)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def SummaryWriter(shared_name, container):
  return summary_writer(shared_name=shared_name, container=container)
tf_export("raw_ops.SummaryWriter")(SummaryWriter)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def WriteAudioSummary(writer, step, tag, tensor, sample_rate, max_outputs):
  return write_audio_summary(writer=writer, step=step, tag=tag, tensor=tensor, sample_rate=sample_rate, max_outputs=max_outputs)
tf_export("raw_ops.WriteAudioSummary")(WriteAudioSummary)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def WriteGraphSummary(writer, step, tensor):
  return write_graph_summary(writer=writer, step=step, tensor=tensor)
tf_export("raw_ops.WriteGraphSummary")(WriteGraphSummary)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def WriteHistogramSummary(writer, step, tag, values):
  return write_histogram_summary(writer=writer, step=step, tag=tag, values=values)
tf_export("raw_ops.WriteHistogramSummary")(WriteHistogramSummary)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def WriteImageSummary(writer, step, tag, tensor, bad_color, max_images):
  return write_image_summary(writer=writer, step=step, tag=tag, tensor=tensor, bad_color=bad_color, max_images=max_images)
tf_export("raw_ops.WriteImageSummary")(WriteImageSummary)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def WriteScalarSummary(writer, step, tag, value):
  return write_scalar_summary(writer=writer, step=step, tag=tag, value=value)
tf_export("raw_ops.WriteScalarSummary")(WriteScalarSummary)
...
def write_summary(writer, step, tensor, tag, summary_metadata, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "WriteSummary", name, _ctx._post_execution_callbacks, writer, step,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def WriteSummary(writer, step, tensor, tag, summary_metadata):
  return write_summary(writer=writer, step=step, tensor=tensor, tag=tag, summary_metadata=summary_metadata)
tf_export("raw_ops.WriteSummary")(WriteSummary)

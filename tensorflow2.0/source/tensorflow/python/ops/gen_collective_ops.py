...
@_kwarg_only
def CollectiveBcastRecv(T, group_size, group_key, instance_key, shape):
  return collective_bcast_recv(T=T, group_size=group_size, group_key=group_key, instance_key=instance_key, shape=shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CollectiveBcastSend(input, group_size, group_key, instance_key, shape):
  return collective_bcast_send(input=input, group_size=group_size, group_key=group_key, instance_key=instance_key, shape=shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CollectiveGather(input, group_size, group_key, instance_key, shape):
  return collective_gather(input=input, group_size=group_size, group_key=group_key, instance_key=instance_key, shape=shape)
...
def collective_reduce(input, group_size, group_key, instance_key, merge_op, final_op, subdiv_offsets, wait_for=[], name=None):
    ...
    try:
      ...
      try:
        return collective_reduce_eager_fallback(
            ...
            subdiv_offsets=subdiv_offsets, wait_for=wait_for, name=name,
            ctx=_ctx)
      except _core._SymbolicException:
        pass  # Add nodes to the TensorFlow graph.
    except _core._NotOkStatusException as e:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CollectiveReduce(input, group_size, group_key, instance_key, merge_op, final_op, subdiv_offsets, wait_for):
  return collective_reduce(input=input, group_size=group_size, group_key=group_key, instance_key=instance_key, merge_op=merge_op, final_op=final_op, subdiv_offsets=subdiv_offsets, wait_for=wait_for)
tf_export("raw_ops.CollectiveReduce")(CollectiveReduce)


def collective_reduce_eager_fallback(input, group_size, group_key, instance_key, merge_op, final_op, subdiv_offsets, wait_for=[], name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.

  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        "EagerPyFunc", name, _ctx._post_execution_callbacks, input, "token",
        token, "Tout", Tout)
      return _result
...
@_kwarg_only
def EagerPyFunc(input, token, Tout):
  return eager_py_func(input=input, token=token, Tout=Tout)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def PyFunc(input, token, Tout):
  return py_func(input=input, token=token, Tout=Tout)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def PyFuncStateless(input, token, Tout):
  return py_func_stateless(input=input, token=token, Tout=Tout)

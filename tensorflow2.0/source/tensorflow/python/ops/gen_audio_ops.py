...
@_kwarg_only
def AudioSpectrogram(input, window_size, stride, magnitude_squared):
  return audio_spectrogram(input=input, window_size=window_size, stride=stride, magnitude_squared=magnitude_squared)
tf_export("raw_ops.AudioSpectrogram")(AudioSpectrogram)
...
def decode_wav(contents, desired_channels=-1, desired_samples=-1, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "DecodeWav", name, _ctx._post_execution_callbacks, contents,
        "desired_channels", desired_channels, "desired_samples",
        desired_samples)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DecodeWav(contents, desired_channels, desired_samples):
  return decode_wav(contents=contents, desired_channels=desired_channels, desired_samples=desired_samples)
tf_export("raw_ops.DecodeWav")(DecodeWav)
...
def encode_wav(audio, sample_rate, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "EncodeWav", name, _ctx._post_execution_callbacks, audio, sample_rate)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def EncodeWav(audio, sample_rate):
  return encode_wav(audio=audio, sample_rate=sample_rate)
tf_export("raw_ops.EncodeWav")(EncodeWav)
...
def mfcc(spectrogram, sample_rate, upper_frequency_limit=4000, lower_frequency_limit=20, filterbank_channel_count=40, dct_coefficient_count=13, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name, "Mfcc",
        name, _ctx._post_execution_callbacks, spectrogram, sample_rate,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def Mfcc(spectrogram, sample_rate, upper_frequency_limit, lower_frequency_limit, filterbank_channel_count, dct_coefficient_count):
  return mfcc(spectrogram=spectrogram, sample_rate=sample_rate, upper_frequency_limit=upper_frequency_limit, lower_frequency_limit=lower_frequency_limit, filterbank_channel_count=filterbank_channel_count, dct_coefficient_count=dct_coefficient_count)
tf_export("raw_ops.Mfcc")(Mfcc)

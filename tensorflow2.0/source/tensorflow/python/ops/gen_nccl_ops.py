...
@_kwarg_only
def NcclAllReduce(input, reduction, num_devices, shared_name):
  return nccl_all_reduce(input=input, reduction=reduction, num_devices=num_devices, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def NcclBroadcast(input, shape):
  return nccl_broadcast(input=input, shape=shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def NcclReduce(input, reduction):
  return nccl_reduce(input=input, reduction=reduction)

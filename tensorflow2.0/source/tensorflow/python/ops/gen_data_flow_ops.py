...
@_kwarg_only
def AccumulatorApplyGradient(handle, local_step, gradient):
  return accumulator_apply_gradient(handle=handle, local_step=local_step, gradient=gradient)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def AccumulatorNumAccumulated(handle):
  return accumulator_num_accumulated(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def AccumulatorSetGlobalStep(handle, new_global_step):
  return accumulator_set_global_step(handle=handle, new_global_step=new_global_step)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def AccumulatorTakeGradient(handle, num_required, dtype):
  return accumulator_take_gradient(handle=handle, num_required=num_required, dtype=dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def Barrier(component_types, shapes, capacity, container, shared_name):
  return barrier(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def BarrierClose(handle, cancel_pending_enqueues):
  return barrier_close(handle=handle, cancel_pending_enqueues=cancel_pending_enqueues)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def BarrierIncompleteSize(handle):
  return barrier_incomplete_size(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def BarrierInsertMany(handle, keys, values, component_index):
  return barrier_insert_many(handle=handle, keys=keys, values=values, component_index=component_index)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def BarrierReadySize(handle):
  return barrier_ready_size(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def BarrierTakeMany(handle, num_elements, component_types, allow_small_batch, wait_for_incomplete, timeout_ms):
  return barrier_take_many(handle=handle, num_elements=num_elements, component_types=component_types, allow_small_batch=allow_small_batch, wait_for_incomplete=wait_for_incomplete, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ConditionalAccumulator(dtype, shape, container, shared_name, reduction_type):
  return conditional_accumulator(dtype=dtype, shape=shape, container=container, shared_name=shared_name, reduction_type=reduction_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DeleteSessionTensor(handle):
  return delete_session_tensor(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DynamicPartition(data, partitions, num_partitions):
  return dynamic_partition(data=data, partitions=partitions, num_partitions=num_partitions)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DynamicStitch(indices, data):
  return dynamic_stitch(indices=indices, data=data)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def FIFOQueue(component_types, shapes, capacity, container, shared_name):
  return fifo_queue(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def FIFOQueueV2(component_types, shapes, capacity, container, shared_name):
  return fifo_queue_v2(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def FakeQueue(resource):
  return fake_queue(resource=resource)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def GetSessionHandle(value):
  return get_session_handle(value=value)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def GetSessionHandleV2(value):
  return get_session_handle_v2(value=value)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def GetSessionTensor(handle, dtype):
  return get_session_tensor(handle=handle, dtype=dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def MapClear(capacity, memory_limit, dtypes, container, shared_name):
  return map_clear(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def MapIncompleteSize(capacity, memory_limit, dtypes, container, shared_name):
  return map_incomplete_size(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def MapPeek(key, indices, capacity, memory_limit, dtypes, container, shared_name):
  return map_peek(key=key, indices=indices, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def MapSize(capacity, memory_limit, dtypes, container, shared_name):
  return map_size(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def MapStage(key, indices, values, capacity, memory_limit, dtypes, container, shared_name):
  return map_stage(key=key, indices=indices, values=values, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def MapUnstage(key, indices, capacity, memory_limit, dtypes, container, shared_name):
  return map_unstage(key=key, indices=indices, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def MapUnstageNoKey(indices, capacity, memory_limit, dtypes, container, shared_name):
  return map_unstage_no_key(indices=indices, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def OrderedMapClear(capacity, memory_limit, dtypes, container, shared_name):
  return ordered_map_clear(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def OrderedMapIncompleteSize(capacity, memory_limit, dtypes, container, shared_name):
  return ordered_map_incomplete_size(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def OrderedMapPeek(key, indices, capacity, memory_limit, dtypes, container, shared_name):
  return ordered_map_peek(key=key, indices=indices, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def OrderedMapSize(capacity, memory_limit, dtypes, container, shared_name):
  return ordered_map_size(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def OrderedMapStage(key, indices, values, capacity, memory_limit, dtypes, container, shared_name):
  return ordered_map_stage(key=key, indices=indices, values=values, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def OrderedMapUnstage(key, indices, capacity, memory_limit, dtypes, container, shared_name):
  return ordered_map_unstage(key=key, indices=indices, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def OrderedMapUnstageNoKey(indices, capacity, memory_limit, dtypes, container, shared_name):
  return ordered_map_unstage_no_key(indices=indices, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def PaddingFIFOQueue(component_types, shapes, capacity, container, shared_name):
  return padding_fifo_queue(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def PaddingFIFOQueueV2(component_types, shapes, capacity, container, shared_name):
  return padding_fifo_queue_v2(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ParallelDynamicStitch(indices, data):
  return parallel_dynamic_stitch(indices=indices, data=data)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def PriorityQueue(component_types, shapes, capacity, container, shared_name):
  return priority_queue(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def PriorityQueueV2(component_types, shapes, capacity, container, shared_name):
  return priority_queue_v2(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueClose(handle, cancel_pending_enqueues):
  return queue_close(handle=handle, cancel_pending_enqueues=cancel_pending_enqueues)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueCloseV2(handle, cancel_pending_enqueues):
  return queue_close_v2(handle=handle, cancel_pending_enqueues=cancel_pending_enqueues)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueDequeue(handle, component_types, timeout_ms):
  return queue_dequeue(handle=handle, component_types=component_types, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueDequeueMany(handle, n, component_types, timeout_ms):
  return queue_dequeue_many(handle=handle, n=n, component_types=component_types, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueDequeueManyV2(handle, n, component_types, timeout_ms):
  return queue_dequeue_many_v2(handle=handle, n=n, component_types=component_types, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueDequeueUpTo(handle, n, component_types, timeout_ms):
  return queue_dequeue_up_to(handle=handle, n=n, component_types=component_types, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueDequeueUpToV2(handle, n, component_types, timeout_ms):
  return queue_dequeue_up_to_v2(handle=handle, n=n, component_types=component_types, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueDequeueV2(handle, component_types, timeout_ms):
  return queue_dequeue_v2(handle=handle, component_types=component_types, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueEnqueue(handle, components, timeout_ms):
  return queue_enqueue(handle=handle, components=components, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueEnqueueMany(handle, components, timeout_ms):
  return queue_enqueue_many(handle=handle, components=components, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueEnqueueManyV2(handle, components, timeout_ms):
  return queue_enqueue_many_v2(handle=handle, components=components, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueEnqueueV2(handle, components, timeout_ms):
  return queue_enqueue_v2(handle=handle, components=components, timeout_ms=timeout_ms)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueIsClosed(handle):
  return queue_is_closed(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueIsClosedV2(handle):
  return queue_is_closed_v2(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueSize(handle):
  return queue_size(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QueueSizeV2(handle):
  return queue_size_v2(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RandomShuffleQueue(component_types, shapes, capacity, min_after_dequeue, seed, seed2, container, shared_name):
  return random_shuffle_queue(component_types=component_types, shapes=shapes, capacity=capacity, min_after_dequeue=min_after_dequeue, seed=seed, seed2=seed2, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RandomShuffleQueueV2(component_types, shapes, capacity, min_after_dequeue, seed, seed2, container, shared_name):
  return random_shuffle_queue_v2(component_types=component_types, shapes=shapes, capacity=capacity, min_after_dequeue=min_after_dequeue, seed=seed, seed2=seed2, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RecordInput(file_pattern, file_random_seed, file_shuffle_shift_ratio, file_buffer_size, file_parallelism, batch_size, compression_type):
  return record_input(file_pattern=file_pattern, file_random_seed=file_random_seed, file_shuffle_shift_ratio=file_shuffle_shift_ratio, file_buffer_size=file_buffer_size, file_parallelism=file_parallelism, batch_size=batch_size, compression_type=compression_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def SparseAccumulatorApplyGradient(handle, local_step, gradient_indices, gradient_values, gradient_shape, has_known_shape):
  return sparse_accumulator_apply_gradient(handle=handle, local_step=local_step, gradient_indices=gradient_indices, gradient_values=gradient_values, gradient_shape=gradient_shape, has_known_shape=has_known_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def SparseAccumulatorTakeGradient(handle, num_required, dtype):
  return sparse_accumulator_take_gradient(handle=handle, num_required=num_required, dtype=dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def SparseConditionalAccumulator(dtype, shape, container, shared_name, reduction_type):
  return sparse_conditional_accumulator(dtype=dtype, shape=shape, container=container, shared_name=shared_name, reduction_type=reduction_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def Stack(elem_type, stack_name):
  return _stack(elem_type=elem_type, stack_name=stack_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StackClose(handle):
  return stack_close(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StackCloseV2(handle):
  return stack_close_v2(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StackPop(handle, elem_type):
  return stack_pop(handle=handle, elem_type=elem_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StackPopV2(handle, elem_type):
  return stack_pop_v2(handle=handle, elem_type=elem_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StackPush(handle, elem, swap_memory):
  return stack_push(handle=handle, elem=elem, swap_memory=swap_memory)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StackPushV2(handle, elem, swap_memory):
  return stack_push_v2(handle=handle, elem=elem, swap_memory=swap_memory)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StackV2(max_size, elem_type, stack_name):
  return stack_v2(max_size=max_size, elem_type=elem_type, stack_name=stack_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def Stage(values, capacity, memory_limit, container, shared_name):
  return stage(values=values, capacity=capacity, memory_limit=memory_limit, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StageClear(capacity, memory_limit, dtypes, container, shared_name):
  return stage_clear(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StagePeek(index, capacity, memory_limit, dtypes, container, shared_name):
  return stage_peek(index=index, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StageSize(capacity, memory_limit, dtypes, container, shared_name):
  return stage_size(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArray(size, dtype, dynamic_size, clear_after_read, tensor_array_name, element_shape):
  return tensor_array(size=size, dtype=dtype, dynamic_size=dynamic_size, clear_after_read=clear_after_read, tensor_array_name=tensor_array_name, element_shape=element_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayClose(handle):
  return tensor_array_close(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayCloseV2(handle):
  return tensor_array_close_v2(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayCloseV3(handle):
  return tensor_array_close_v3(handle=handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayConcat(handle, flow_in, dtype, element_shape_except0):
  return tensor_array_concat(handle=handle, flow_in=flow_in, dtype=dtype, element_shape_except0=element_shape_except0)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayConcatV2(handle, flow_in, dtype, element_shape_except0):
  return tensor_array_concat_v2(handle=handle, flow_in=flow_in, dtype=dtype, element_shape_except0=element_shape_except0)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayConcatV3(handle, flow_in, dtype, element_shape_except0):
  return tensor_array_concat_v3(handle=handle, flow_in=flow_in, dtype=dtype, element_shape_except0=element_shape_except0)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayGather(handle, indices, flow_in, dtype, element_shape):
  return tensor_array_gather(handle=handle, indices=indices, flow_in=flow_in, dtype=dtype, element_shape=element_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayGatherV2(handle, indices, flow_in, dtype, element_shape):
  return tensor_array_gather_v2(handle=handle, indices=indices, flow_in=flow_in, dtype=dtype, element_shape=element_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayGatherV3(handle, indices, flow_in, dtype, element_shape):
  return tensor_array_gather_v3(handle=handle, indices=indices, flow_in=flow_in, dtype=dtype, element_shape=element_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayGrad(handle, flow_in, source):
  return tensor_array_grad(handle=handle, flow_in=flow_in, source=source)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayGradV2(handle, flow_in, source):
  return tensor_array_grad_v2(handle=handle, flow_in=flow_in, source=source)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayGradV3(handle, flow_in, source):
  return tensor_array_grad_v3(handle=handle, flow_in=flow_in, source=source)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayGradWithShape(handle, flow_in, shape_to_prepend, source):
  return tensor_array_grad_with_shape(handle=handle, flow_in=flow_in, shape_to_prepend=shape_to_prepend, source=source)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayPack(handle, flow_in, dtype, element_shape):
  return tensor_array_pack(handle=handle, flow_in=flow_in, dtype=dtype, element_shape=element_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayRead(handle, index, flow_in, dtype):
  return tensor_array_read(handle=handle, index=index, flow_in=flow_in, dtype=dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayReadV2(handle, index, flow_in, dtype):
  return tensor_array_read_v2(handle=handle, index=index, flow_in=flow_in, dtype=dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayReadV3(handle, index, flow_in, dtype):
  return tensor_array_read_v3(handle=handle, index=index, flow_in=flow_in, dtype=dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayScatter(handle, indices, value, flow_in):
  return tensor_array_scatter(handle=handle, indices=indices, value=value, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayScatterV2(handle, indices, value, flow_in):
  return tensor_array_scatter_v2(handle=handle, indices=indices, value=value, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayScatterV3(handle, indices, value, flow_in):
  return tensor_array_scatter_v3(handle=handle, indices=indices, value=value, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArraySize(handle, flow_in):
  return tensor_array_size(handle=handle, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArraySizeV2(handle, flow_in):
  return tensor_array_size_v2(handle=handle, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArraySizeV3(handle, flow_in):
  return tensor_array_size_v3(handle=handle, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArraySplit(handle, value, lengths, flow_in):
  return tensor_array_split(handle=handle, value=value, lengths=lengths, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArraySplitV2(handle, value, lengths, flow_in):
  return tensor_array_split_v2(handle=handle, value=value, lengths=lengths, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArraySplitV3(handle, value, lengths, flow_in):
  return tensor_array_split_v3(handle=handle, value=value, lengths=lengths, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayUnpack(handle, value, flow_in):
  return tensor_array_unpack(handle=handle, value=value, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayV2(size, dtype, element_shape, dynamic_size, clear_after_read, tensor_array_name):
  return tensor_array_v2(size=size, dtype=dtype, element_shape=element_shape, dynamic_size=dynamic_size, clear_after_read=clear_after_read, tensor_array_name=tensor_array_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayV3(size, dtype, element_shape, dynamic_size, clear_after_read, identical_element_shapes, tensor_array_name):
  return tensor_array_v3(size=size, dtype=dtype, element_shape=element_shape, dynamic_size=dynamic_size, clear_after_read=clear_after_read, identical_element_shapes=identical_element_shapes, tensor_array_name=tensor_array_name)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayWrite(handle, index, value, flow_in):
  return tensor_array_write(handle=handle, index=index, value=value, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayWriteV2(handle, index, value, flow_in):
  return tensor_array_write_v2(handle=handle, index=index, value=value, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorArrayWriteV3(handle, index, value, flow_in):
  return tensor_array_write_v3(handle=handle, index=index, value=value, flow_in=flow_in)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def Unstage(capacity, memory_limit, dtypes, container, shared_name):
  return unstage(capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name)

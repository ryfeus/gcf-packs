...
@_kwarg_only
def AsString(input, precision, scientific, shortest, width, fill):
  return as_string(input=input, precision=precision, scientific=scientific, shortest=shortest, width=width, fill=fill)
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        "DecodeBase64", name, _ctx._post_execution_callbacks, input)
      return _result
    except _core._FallbackException:
      try:
...
def encode_base64(input, pad=False, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "EncodeBase64", name, _ctx._post_execution_callbacks, input, "pad",
        pad)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def EncodeBase64(input, pad):
  return encode_base64(input=input, pad=pad)
...
def reduce_join(inputs, reduction_indices, keep_dims=False, separator="", name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "ReduceJoin", name, _ctx._post_execution_callbacks, inputs,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ReduceJoin(inputs, reduction_indices, keep_dims, separator):
  return reduce_join(inputs=inputs, reduction_indices=reduction_indices, keep_dims=keep_dims, separator=separator)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RegexFullMatch(input, pattern):
  return regex_full_match(input=input, pattern=pattern)
...
def regex_replace(input, pattern, rewrite, replace_global=True, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "RegexReplace", name, _ctx._post_execution_callbacks, input, pattern,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RegexReplace(input, pattern, rewrite, replace_global):
  return regex_replace(input=input, pattern=pattern, rewrite=rewrite, replace_global=replace_global)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StaticRegexFullMatch(input, pattern):
  return static_regex_full_match(input=input, pattern=pattern)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StaticRegexReplace(input, pattern, rewrite, replace_global):
  return static_regex_replace(input=input, pattern=pattern, rewrite=rewrite, replace_global=replace_global)
...
def string_format(inputs, template="%s", placeholder="%s", summarize=3, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "StringFormat", name, _ctx._post_execution_callbacks, inputs,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringFormat(inputs, template, placeholder, summarize):
  return string_format(inputs=inputs, template=template, placeholder=placeholder, summarize=summarize)
...
def string_join(inputs, separator="", name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "StringJoin", name, _ctx._post_execution_callbacks, inputs,
        "separator", separator)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringJoin(inputs, separator):
  return string_join(inputs=inputs, separator=separator)
...
def string_length(input, unit="BYTE", name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "StringLength", name, _ctx._post_execution_callbacks, input, "unit",
        unit)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringLength(input, unit):
  return string_length(input=input, unit=unit)
...
def string_split(input, delimiter, skip_empty=True, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "StringSplit", name, _ctx._post_execution_callbacks, input, delimiter,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringSplit(input, delimiter, skip_empty):
  return string_split(input=input, delimiter=delimiter, skip_empty=skip_empty)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringSplitV2(input, sep, maxsplit):
  return string_split_v2(input=input, sep=sep, maxsplit=maxsplit)
...
def string_strip(input, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "StringStrip", name, _ctx._post_execution_callbacks, input)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringStrip(input):
  return string_strip(input=input)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringToHashBucket(string_tensor, num_buckets):
  return string_to_hash_bucket(string_tensor=string_tensor, num_buckets=num_buckets)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringToHashBucketFast(input, num_buckets):
  return string_to_hash_bucket_fast(input=input, num_buckets=num_buckets)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StringToHashBucketStrong(input, num_buckets, key):
  return string_to_hash_bucket_strong(input=input, num_buckets=num_buckets, key=key)
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        name, _ctx._post_execution_callbacks, input, pos, len, "unit", unit)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def Substr(input, pos, len, unit):
  return substr(input=input, pos=pos, len=len, unit=unit)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def UnicodeDecode(input, input_encoding, errors, replacement_char, replace_control_characters):
  return unicode_decode(input=input, input_encoding=input_encoding, errors=errors, replacement_char=replacement_char, replace_control_characters=replace_control_characters)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def UnicodeDecodeWithOffsets(input, input_encoding, errors, replacement_char, replace_control_characters):
  return unicode_decode_with_offsets(input=input, input_encoding=input_encoding, errors=errors, replacement_char=replacement_char, replace_control_characters=replace_control_characters)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def UnicodeEncode(input_values, input_splits, errors, output_encoding, replacement_char):
  return unicode_encode(input_values=input_values, input_splits=input_splits, errors=errors, output_encoding=output_encoding, replacement_char=replacement_char)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def UnicodeScript(input):
  return unicode_script(input=input)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def UnicodeTranscode(input, input_encoding, output_encoding, errors, replacement_char, replace_control_characters):
  return unicode_transcode(input=input, input_encoding=input_encoding, output_encoding=output_encoding, errors=errors, replacement_char=replacement_char, replace_control_characters=replace_control_characters)

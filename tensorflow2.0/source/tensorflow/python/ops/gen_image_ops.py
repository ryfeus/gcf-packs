...
@_kwarg_only
def AdjustContrast(images, contrast_factor, min_value, max_value):
  return adjust_contrast(images=images, contrast_factor=contrast_factor, min_value=min_value, max_value=max_value)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def AdjustContrastv2(images, contrast_factor):
  return adjust_contrastv2(images=images, contrast_factor=contrast_factor)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def AdjustHue(images, delta):
  return adjust_hue(images=images, delta=delta)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def AdjustSaturation(images, scale):
  return adjust_saturation(images=images, scale=scale)
...
@tf_export('combined_non_max_suppression')
def combined_non_max_suppression(boxes, scores, max_output_size_per_class, max_total_size, iou_threshold, score_threshold, pad_per_class=False, name=None):
    ...
    try:
      ...
      try:
        ...
        result = _dispatch.dispatch(
              combined_non_max_suppression, boxes=boxes, scores=scores,
  ...
  try:
    _, _, _op = _op_def_lib._apply_op_helper(
        "CombinedNonMaxSuppression", boxes=boxes, scores=scores,
                                     ...
                                     iou_threshold=iou_threshold,
                                     score_threshold=score_threshold,
                                     pad_per_class=pad_per_class, name=name)
  except (TypeError, ValueError):
    result = _dispatch.dispatch(
          combined_non_max_suppression, boxes=boxes, scores=scores,
                                        max_output_size_per_class=max_output_size_per_class,
  ...
  _result = _op.outputs[:]
  _inputs_flat = _op.inputs
  _attrs = ("pad_per_class", _op.get_attr("pad_per_class"))
  _execute.record_gradient(
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CombinedNonMaxSuppression(boxes, scores, max_output_size_per_class, max_total_size, iou_threshold, score_threshold, pad_per_class):
  return combined_non_max_suppression(boxes=boxes, scores=scores, max_output_size_per_class=max_output_size_per_class, max_total_size=max_total_size, iou_threshold=iou_threshold, score_threshold=score_threshold, pad_per_class=pad_per_class)
tf_export("raw_ops.CombinedNonMaxSuppression")(CombinedNonMaxSuppression)


def combined_non_max_suppression_eager_fallback(boxes, scores, max_output_size_per_class, max_total_size, iou_threshold, score_threshold, pad_per_class=False, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  ...
  _attrs = ("pad_per_class", pad_per_class)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CropAndResize(image, boxes, box_ind, crop_size, method, extrapolation_value):
  return crop_and_resize(image=image, boxes=boxes, box_ind=box_ind, crop_size=crop_size, method=method, extrapolation_value=extrapolation_value)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CropAndResizeGradBoxes(grads, image, boxes, box_ind, method):
  return crop_and_resize_grad_boxes(grads=grads, image=image, boxes=boxes, box_ind=box_ind, method=method)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def CropAndResizeGradImage(grads, boxes, box_ind, image_size, T, method):
  return crop_and_resize_grad_image(grads=grads, boxes=boxes, box_ind=box_ind, image_size=image_size, T=T, method=method)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DecodeAndCropJpeg(contents, crop_window, channels, ratio, fancy_upscaling, try_recover_truncated, acceptable_fraction, dct_method):
  return decode_and_crop_jpeg(contents=contents, crop_window=crop_window, channels=channels, ratio=ratio, fancy_upscaling=fancy_upscaling, try_recover_truncated=try_recover_truncated, acceptable_fraction=acceptable_fraction, dct_method=dct_method)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DecodeBmp(contents, channels):
  return decode_bmp(contents=contents, channels=channels)
...
def DecodeGif(contents):
  return decode_gif(contents=contents)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DecodeJpeg(contents, channels, ratio, fancy_upscaling, try_recover_truncated, acceptable_fraction, dct_method):
  return decode_jpeg(contents=contents, channels=channels, ratio=ratio, fancy_upscaling=fancy_upscaling, try_recover_truncated=try_recover_truncated, acceptable_fraction=acceptable_fraction, dct_method=dct_method)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def DecodePng(contents, channels, dtype):
  return decode_png(contents=contents, channels=channels, dtype=dtype)
...
def DrawBoundingBoxes(images, boxes):
  return draw_bounding_boxes(images=images, boxes=boxes)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def EncodeJpeg(image, format, quality, progressive, optimize_size, chroma_downsampling, density_unit, x_density, y_density, xmp_metadata):
  return encode_jpeg(image=image, format=format, quality=quality, progressive=progressive, optimize_size=optimize_size, chroma_downsampling=chroma_downsampling, density_unit=density_unit, x_density=x_density, y_density=y_density, xmp_metadata=xmp_metadata)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def EncodePng(image, compression):
  return encode_png(image=image, compression=compression)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ExtractGlimpse(input, size, offsets, centered, normalized, uniform_noise, noise):
  return extract_glimpse(input=input, size=size, offsets=offsets, centered=centered, normalized=normalized, uniform_noise=uniform_noise, noise=noise)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ExtractJpegShape(contents, output_type):
  return extract_jpeg_shape(contents=contents, output_type=output_type)
...
@_kwarg_only
def HSVToRGB(images):
  return hsv_to_rgb(images=images)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def NonMaxSuppression(boxes, scores, max_output_size, iou_threshold):
  return non_max_suppression(boxes=boxes, scores=scores, max_output_size=max_output_size, iou_threshold=iou_threshold)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def NonMaxSuppressionV2(boxes, scores, max_output_size, iou_threshold):
  return non_max_suppression_v2(boxes=boxes, scores=scores, max_output_size=max_output_size, iou_threshold=iou_threshold)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def NonMaxSuppressionV3(boxes, scores, max_output_size, iou_threshold, score_threshold):
  return non_max_suppression_v3(boxes=boxes, scores=scores, max_output_size=max_output_size, iou_threshold=iou_threshold, score_threshold=score_threshold)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def NonMaxSuppressionV4(boxes, scores, max_output_size, iou_threshold, score_threshold, pad_to_max_output_size):
  return non_max_suppression_v4(boxes=boxes, scores=scores, max_output_size=max_output_size, iou_threshold=iou_threshold, score_threshold=score_threshold, pad_to_max_output_size=pad_to_max_output_size)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def NonMaxSuppressionWithOverlaps(overlaps, scores, max_output_size, overlap_threshold, score_threshold):
  return non_max_suppression_with_overlaps(overlaps=overlaps, scores=scores, max_output_size=max_output_size, overlap_threshold=overlap_threshold, score_threshold=score_threshold)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def QuantizedResizeBilinear(images, size, min, max, align_corners, half_pixel_centers):
  return quantized_resize_bilinear(images=images, size=size, min=min, max=max, align_corners=align_corners, half_pixel_centers=half_pixel_centers)
...
@_kwarg_only
def RGBToHSV(images):
  return rgb_to_hsv(images=images)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RandomCrop(image, size, seed, seed2):
  return random_crop(image=image, size=size, seed=seed, seed2=seed2)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ResizeArea(images, size, align_corners):
  return resize_area(images=images, size=size, align_corners=align_corners)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ResizeBicubic(images, size, align_corners, half_pixel_centers):
  return resize_bicubic(images=images, size=size, align_corners=align_corners, half_pixel_centers=half_pixel_centers)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ResizeBicubicGrad(grads, original_image, align_corners, half_pixel_centers):
  return resize_bicubic_grad(grads=grads, original_image=original_image, align_corners=align_corners, half_pixel_centers=half_pixel_centers)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ResizeBilinear(images, size, align_corners, half_pixel_centers):
  return resize_bilinear(images=images, size=size, align_corners=align_corners, half_pixel_centers=half_pixel_centers)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ResizeBilinearGrad(grads, original_image, align_corners, half_pixel_centers):
  return resize_bilinear_grad(grads=grads, original_image=original_image, align_corners=align_corners, half_pixel_centers=half_pixel_centers)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ResizeNearestNeighbor(images, size, align_corners, half_pixel_centers):
  return resize_nearest_neighbor(images=images, size=size, align_corners=align_corners, half_pixel_centers=half_pixel_centers)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ResizeNearestNeighborGrad(grads, size, align_corners, half_pixel_centers):
  return resize_nearest_neighbor_grad(grads=grads, size=size, align_corners=align_corners, half_pixel_centers=half_pixel_centers)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def SampleDistortedBoundingBox(image_size, bounding_boxes, seed, seed2, min_object_covered, aspect_ratio_range, area_range, max_attempts, use_image_if_no_bounding_boxes):
  return sample_distorted_bounding_box(image_size=image_size, bounding_boxes=bounding_boxes, seed=seed, seed2=seed2, min_object_covered=min_object_covered, aspect_ratio_range=aspect_ratio_range, area_range=area_range, max_attempts=max_attempts, use_image_if_no_bounding_boxes=use_image_if_no_bounding_boxes)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def SampleDistortedBoundingBoxV2(image_size, bounding_boxes, min_object_covered, seed, seed2, aspect_ratio_range, area_range, max_attempts, use_image_if_no_bounding_boxes):
  return sample_distorted_bounding_box_v2(image_size=image_size, bounding_boxes=bounding_boxes, min_object_covered=min_object_covered, seed=seed, seed2=seed2, aspect_ratio_range=aspect_ratio_range, area_range=area_range, max_attempts=max_attempts, use_image_if_no_bounding_boxes=use_image_if_no_bounding_boxes)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ScaleAndTranslate(images, size, scale, translation, kernel_type, antialias):
  return scale_and_translate(images=images, size=size, scale=scale, translation=translation, kernel_type=kernel_type, antialias=antialias)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def ScaleAndTranslateGrad(grads, original_image, scale, translation, kernel_type, antialias):
  return scale_and_translate_grad(grads=grads, original_image=original_image, scale=scale, translation=translation, kernel_type=kernel_type, antialias=antialias)

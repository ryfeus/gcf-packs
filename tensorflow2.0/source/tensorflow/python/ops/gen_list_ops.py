...
@_kwarg_only
def EmptyTensorList(element_shape, max_num_elements, element_dtype):
  return empty_tensor_list(element_shape=element_shape, max_num_elements=max_num_elements, element_dtype=element_dtype)
...
def tensor_list_concat(input_handle, element_dtype, element_shape=None, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        input_handle, "element_dtype", element_dtype, "element_shape",
        element_shape)
  ...
  _result = _op.outputs[:]
  _inputs_flat = _op.inputs
  _attrs = ("element_dtype", _op.get_attr("element_dtype"), "element_shape",
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListConcat(input_handle, element_dtype, element_shape):
  return tensor_list_concat(input_handle=input_handle, element_dtype=element_dtype, element_shape=element_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListConcatLists(input_a, input_b, element_dtype):
  return tensor_list_concat_lists(input_a=input_a, input_b=input_b, element_dtype=element_dtype)
...
def tensor_list_concat_v2(input_handle, element_shape, leading_dims, element_dtype, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "TensorListConcatV2", name, _ctx._post_execution_callbacks,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListConcatV2(input_handle, element_shape, leading_dims, element_dtype):
  return tensor_list_concat_v2(input_handle=input_handle, element_shape=element_shape, leading_dims=leading_dims, element_dtype=element_dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListElementShape(input_handle, shape_type):
  return tensor_list_element_shape(input_handle=input_handle, shape_type=shape_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListFromTensor(tensor, element_shape):
  return tensor_list_from_tensor(tensor=tensor, element_shape=element_shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListGather(input_handle, indices, element_shape, element_dtype):
  return tensor_list_gather(input_handle=input_handle, indices=indices, element_shape=element_shape, element_dtype=element_dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListGetItem(input_handle, index, element_shape, element_dtype):
  return tensor_list_get_item(input_handle=input_handle, index=index, element_shape=element_shape, element_dtype=element_dtype)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListLength(input_handle):
  return tensor_list_length(input_handle=input_handle)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListPopBack(input_handle, element_shape, element_dtype):
  return tensor_list_pop_back(input_handle=input_handle, element_shape=element_shape, element_dtype=element_dtype)
...
def TensorListPushBack(input_handle, tensor):
  return tensor_list_push_back(input_handle=input_handle, tensor=tensor)
...
def TensorListPushBackBatch(input_handles, tensor):
  return tensor_list_push_back_batch(input_handles=input_handles, tensor=tensor)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListReserve(element_shape, num_elements, element_dtype):
  return tensor_list_reserve(element_shape=element_shape, num_elements=num_elements, element_dtype=element_dtype)
...
def tensor_list_resize(input_handle, size, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "TensorListResize", name, _ctx._post_execution_callbacks,
        input_handle, size)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListResize(input_handle, size):
  return tensor_list_resize(input_handle=input_handle, size=size)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListScatter(tensor, indices, element_shape):
  return tensor_list_scatter(tensor=tensor, indices=indices, element_shape=element_shape)
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        _ctx._post_execution_callbacks, input_handle, tensor, indices)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListScatterIntoExistingList(input_handle, tensor, indices):
  return tensor_list_scatter_into_existing_list(input_handle=input_handle, tensor=tensor, indices=indices)
...
def tensor_list_scatter_v2(tensor, indices, element_shape, num_elements, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "TensorListScatterV2", name, _ctx._post_execution_callbacks, tensor,
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListScatterV2(tensor, indices, element_shape, num_elements):
  return tensor_list_scatter_v2(tensor=tensor, indices=indices, element_shape=element_shape, num_elements=num_elements)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListSetItem(input_handle, index, item):
  return tensor_list_set_item(input_handle=input_handle, index=index, item=item)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListSplit(tensor, element_shape, lengths):
  return tensor_list_split(tensor=tensor, element_shape=element_shape, lengths=lengths)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def TensorListStack(input_handle, element_shape, element_dtype, num_elements):
  return tensor_list_stack(input_handle=input_handle, element_shape=element_shape, element_dtype=element_dtype, num_elements=num_elements)

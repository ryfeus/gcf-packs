...
@_kwarg_only
def Case(branch_index, input, Tout, branches, output_shapes):
  return case(branch_index=branch_index, input=input, Tout=Tout, branches=branches, output_shapes=output_shapes)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def FakeParam(dtype, shape):
  return fake_param(dtype=dtype, shape=shape)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def For(start, limit, delta, input, body):
  return _for(start=start, limit=limit, delta=delta, input=input, body=body)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def If(cond, input, Tout, then_branch, else_branch, output_shapes):
  return _if(cond=cond, input=input, Tout=Tout, then_branch=then_branch, else_branch=else_branch, output_shapes=output_shapes)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def PartitionedCall(args, Tout, f, config, config_proto, executor_type):
  return partitioned_call(args=args, Tout=Tout, f=f, config=config, config_proto=config_proto, executor_type=executor_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RemoteCall(target, args, Tout, f):
  return remote_call(target=target, args=args, Tout=Tout, f=f)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StatefulPartitionedCall(args, Tout, f, config, config_proto, executor_type):
  return stateful_partitioned_call(args=args, Tout=Tout, f=f, config=config, config_proto=config_proto, executor_type=executor_type)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StatelessIf(cond, input, Tout, then_branch, else_branch):
  return stateless_if(cond=cond, input=input, Tout=Tout, then_branch=then_branch, else_branch=else_branch)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def StatelessWhile(input, cond, body):
  return stateless_while(input=input, cond=cond, body=body)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def SymbolicGradient(input, Tout, f):
  return symbolic_gradient(input=input, Tout=Tout, f=f)
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def While(input, cond, body, output_shapes, parallel_iterations):
  return _while(input=input, cond=cond, body=body, output_shapes=output_shapes, parallel_iterations=parallel_iterations)
